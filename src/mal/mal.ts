...
// READ
export function read(str: string): MalType {
  return readStr(str);
}
...
// EVAL
export function evalMal(ast: MalType, env: Env): MalType {
  loop: while (true) {
        ...
        switch (first.v) {
          ...
          case "try*": {
            try {
              ...
              if (!isSeq(catchBody)) {
                throw new Error(
                  `unexpected return type: ${catchBody.type}, expected: list or vector`
                );
              }
              const catchSymbol = catchBody.list[0];
              if (
                catchSymbol.type === Node.Symbol &&
                catchSymbol.v === "catch*"
              ) {
                const errorSymbol = catchBody.list[1];
                if (errorSymbol.type !== Node.Symbol) {
                  throw new Error(
                    `unexpected return type: ${errorSymbol.type}, expected: symbol`
                  );
                }
                if (!isAST(e)) {
          ...
          case "fn*": {
            ...
            if (!isSeq(params)) {
              throw new Error(
                `unexpected return type: ${params.type}, expected: list or vector`
              );
            }
 ...
 * Run script
 */
export function evalScript(scope: Record<string, any>, script: string): any {
  const env = new Env();
  core.consts.forEach((value, key) => {
    env.set(key, value);
  });
  ...
  for (let name in scope) {
    env.set(MalSymbol.get(name), js_to_mal(scope[name]));
  }
  const result = evalMal(read(script), env);
  return mal_to_js(result);
}
