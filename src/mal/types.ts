  ...
  if (a.type === Node.HashMap && b.type === Node.HashMap) {
    ...
    for (const [aK, aV] of a.entries()) {
      if (aK.type !== Node.String && aK.type !== Node.Keyword) {
        throw new Error(
          `unexpected symbol: ${aK.type}, expected: string or keyword`
        );
      }
      const bV = b.get(aK);
...
export class MalHashMap {
  ...
  constructor(list: MalType[]) {
    while (list.length !== 0) {
      ...
      if (key.type === Node.Keyword) {
        ...
        throw new Error(
          `unexpected key symbol: ${key.type}, expected: keyword or string`
        );
      }
    }
  }
  ...
  entries(): [MalType, MalType][] {
    ...
    Object.keys(this.stringMap).forEach((v) =>
      list.push([new MalString(v), this.stringMap[v]])
    );
...
export class MalFunction {
  static fromLisp(
    evalMal: (ast: MalType, env: Env) => MalType,
    env: Env,
    params: MalSymbol[],
    bodyAst: MalType
  ): MalFunction {
...
export class MalJSObject {
  type: Node.JSObject = Node.JSObject;
  meta?: MalType;

  constructor(public obj: any) {}

  withMeta(meta: MalType) {
    const v = new MalJSObject(this.obj);
    v.meta = meta;
    return v;
  }
}
