...
function setHorzAlign(
  ...
  dx += offset;
  changed = tr.moveShapes(doc, [shape], dx, dy);
  if (width > -1) {
    changed =
      ...
      case "top": {
        ...
        for (let child of arr) {
          if (child instanceof Box) {
            ...
            ty = child.bottom;
            changed =
              setHorzAlign(transform, doc, child, shape, args.align) || changed;
            // fill last child
      ...
      case "left": {
        ...
        for (let child of arr) {
          if (child instanceof Box) {
            ...
            lx = child.right;
            changed =
              setVertAlign(transform, doc, child, shape, args.align) || changed;
            // fill last child
            if (args.fillLast && child === arr[arr.length - 1]) {
