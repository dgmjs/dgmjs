...
import { Note, Shape } from "../shapes";
import { addShape, resolveAllConstraints } from "../macro";

/**
 * Note Factory Handler
 */
export class NoteFactoryHandler extends Handler {
  dragging: boolean = false;
  dragStartPoint: number[] = [-1, -1];
  dragPoint: number[] = [-1, -1];
  shape: Note | null = null;
  ...
  initialize(editor: Editor, e: CanvasPointerEvent): void {
    ...
    if (page) {
      this.shape = editor.factory.createNote([
        this.dragStartPoint,
        this.dragPoint,
      ]);
